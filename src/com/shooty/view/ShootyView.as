package com.shooty.view 
{
	...
	public class ShootyView extends EventDispatcher
	{
		...
		public var customEnemyLayer		:PixiLayer;
		...
		public function update() : void 
		{
			...
			if(Cnst.CLOUDS_ENABLED)skyLayer.position = position;
			pixiEngine.render();
		}
		
		public function isHittingPauseButton():Boolean{
			return pauseMenu.pauseButton.hitTestPoint(mouseX*scale, mouseY*scale);
		}
		...
		private function onEngineReady(event : Event) : void
		{
			...
			customEnemyLayer = new PixiLayer("assets/textures/CustomEnemies.png");
			...
			if(Cnst.CLOUDS_ENABLED)skyLayer = new SkyLayer();
			
			pixiEngine.addLayer(floorLayer);
			if(Cnst.CLOUDS_ENABLED)pixiEngine.addLayer(skyLayer);
			pixiEngine.addLayer(customEnemyLayer);
