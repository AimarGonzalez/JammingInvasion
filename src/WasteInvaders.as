package 
{
	...
	public class WasteInvaders extends Sprite 
	{
		...
		public function WasteInvaders() : void 
		{
			...
			this.addEventListener(Event.USER_IDLE, onUserIdleHandler);			
			this.addEventListener(Event.USER_PRESENT, onUserPresentHandler);			
		...
		//	!!!PETARME TODO ESTO GRAFICO Y PONER MI ROLLO!!!
			
			PixiResourceManager.instance.loadTextureAndPlist("assets/textures/basketball");
			PixiResourceManager.instance.loadTextureAndPlist("assets/textures/CustomEnemies");
			if(Cnst.INTRO_ENABLED){
				PixiResourceManager.instance.addEventListener(NetEvent.LOAD_COMPLETE, onAssetsLoaded);
			}else{
				PixiResourceManager.instance.addEventListener(NetEvent.LOAD_COMPLETE, onIntroFinished);
			}
		...
		private function onGameReady(event : Event) : void
		{
			...
			music = SwfAsset.getSound("loveme");
			
		//	var mySong:Sound = new Sound(new URLRequest("loveme.mp3"));
			//mySong.
		...
		private function onTouchReady(event : Event) : void
		{
			if(!Cnst.MICRO_ENABLED){
				music.forceStart(1, 99999);
				//mySong
			}
		...
		private function onActivateHandler(event:Event):void{
			if(shootyEngine && shootyEngine.gamePlaying && shootyEngine.shootyView && shootyEngine.shootyView.pauseMenu){
				trace("ACTIVATE: resume");
			}
		}
		
		private function onDeactivateHandler(event:Event):void{
			if(shootyEngine && shootyEngine.gamePlaying && shootyEngine.shootyView && shootyEngine.shootyView.pauseMenu){
				shootyEngine.shootyView.pauseMenu.pause(true, true);
				trace("DEACTIVATE: pause");
			}
		}
		...
		private function onUserPresentHandler(event:Event):void{
			if(shootyEngine && shootyEngine.gamePlaying && shootyEngine.shootyView && shootyEngine.shootyView.pauseMenu){
				trace("USER_PRESENT: resume");
			}
		}
		
		private function onUserIdleHandler(event:Event):void{
			if(shootyEngine && shootyEngine.gamePlaying && shootyEngine.shootyView && shootyEngine.shootyView.pauseMenu){
				//shootyEngine.shootyView.pauseMenu.pause(true, true);
				trace("USER_IDLE: pause");
			}
		}
